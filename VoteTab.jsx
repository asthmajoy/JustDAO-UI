...
const VoteTab = ({ proposals, castVote, hasVoted, getVotingPower, getProposalVoteTotals, voting, account, governanceContract, provider, contractAddress }) => {
  ...
  const submitVote = async (proposalId, support) => {
    try {
      ...
      const votingPower = votingPowers[proposalId] || 0;
      const powerValue = parseFloat(votingPower);
      ...
      const hasVotedBefore = hasUserVoted(proposal);
      if (!hasVotedBefore) {
      ...
      if (support === VOTE_TYPES.FOR) {
        updatedVoteData.yesVotes += powerValue;
      } else if (support === VOTE_TYPES.AGAINST) {
        updatedVoteData.noVotes += powerValue;
      } else if (support === VOTE_TYPES.ABSTAIN) {
        updatedVoteData.abstainVotes += powerValue;
      }
      ...
      setTimeout(async () => {
        try {
          ...
          if (freshVoteData && freshVoteData.totalVotes > 0) {
            setProposalVotes(prev => ({
              ...prev,
